...
class StegaStampRemover(tf.layers.Layer):
    def __init__(self, secret_size):
        super(StegaStampRemover, self).__init__()
        ...
        self.output_conv = Conv2D(3, 1, activation='sigmoid', padding='same', kernel_initializer='he_normal')
    ...
    def call(self, inputs):
        ...
        secret = tf.reshape(secret, [-1, self.secret_size])
        # secret = tf.Print(secret, [tf.shape(secret)], message="secret shape: ")

        # Dense 레이어 통과 및 형태 출력
        secret_embedding = self.secret_dense(secret)
        # secret_embedding = tf.Print(secret_embedding, [tf.shape(secret_embedding)], message="secret_embedding shape: ")


        secret_embedding = Reshape((50, 50, 3))(secret_embedding)
        secret_enlarged = UpSampling2D(size=(8, 8))(secret_embedding) 

        combined_input = concatenate([image, secret_enlarged], axis=-1)

        # Encoder path
        conv1 = self.conv1(combined_input)
        ...
        # Output layer
        output = self.output_conv(conv10)
        return output
