...
then
    ...
    do
        echo $i $j;
        python calculate_accuracy.py $stegastamp_model \
            --images_dir ${attacked_dir}/${attacked_subfolder_name}_${j}/ 2>/dev/null | grep "Detection Rate";
        python -m pytorch_fid ${attacked_dir}/${attacked_subfolder_name}_${j}/ $unwatermarked_dir | grep "FID";
    done
fi