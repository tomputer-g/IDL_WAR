...
class DiffPure():
    def __init__(self, steps=0.4, save_imgs=False, fname="base"):
        ...
        if self.save_imgs:
            save_dir = f'./regen_image/{self.fname}/{self.steps}'
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
    ...
    def __call__(self, img):
        ...
        # if self.save_imgs:
        #     save_dir = f'./regen_image/{self.fname}/{self.steps}'
        #     # save_image(img, os.path.join(save_dir, fname))
        #     # save_image(img_noisy, os.path.join(save_dir, 'noisy_'+fname))
        #     save_image(img_pured, os.path.join(save_dir, fname))
        return img_pured
...
def main():
    ...
    parser.add_argument("--wm_method", default="dwtDct", type=str,
    ...
    parser.add_argument("--data_dir", default="images/imagenet/dwtDct", type=str)
    parser.add_argument("--org_data_dir", default="images/imagenet/org", type=str)
    parser.add_argument("--dataset", default="imagenet", type=str)
    parser.add_argument("--out_fname", default="out", type=str)
    ...
    for aug in aug_list:
        ...
        dataset = CustomImageFolder(args.data_dir, transform=transform,
                                    save_img=True, step=float(aug.split(',')[-1]))
