...
def apply_adv(args, mode='wm', eps=8):
    ...
    for j in range(j_num):
        for i, (image, target, filenames) in tqdm(enumerate(val_loader), desc="Processing val_loader", total=len(val_loader)):
    ...
    if mode == 'wm':
        images_dir = f'/DATA/CMU/competition/adv_wm_images_{args.wm_method}_{eps}/'
        diffs_dir = f'/DATA/CMU/competition/diff_wm_images_{args.wm_method}_{eps}/'
    else:
        images_dir = f'/DATA/CMU/surrogate_model/adv_images/adv_org_images_{args.wm_method}_{eps}/'
        diffs_dir = f'/DATA/CMU/surrogate_model/adv_images/diff_org_images_{args.wm_method}_{eps}/'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wm_method", default="treeRing", type=str,
        choices=["treeRing", "stegaStamp"])
    parser.add_argument("--wm_dir", type=str, required=True)
    #parser.add_argument("--org_dir", type=str, required=True)
