...
def remove_watermark(attack_method, image, strength, model):
    ...
    if attack_method == "regen_diffusion":
        pipe = ReSDPipeline.from_pretrained(
            model, torch_dtype=torch.float16, revision="fp16", use_safetensors=False
        )
...
def get_output_filename(output_path: str, strength: int, n: int):
    return os.path.join(output_path, "attacked_strength" + str(strength) + "_n" + str(n) + ".png")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_rinse", type=int, default=1)
    parser.add_argument("--strength", type=int, default=10)
    parser.add_argument("--orig_img_size", type=int, default=None)
    ...
    # file list
    file_list = sorted(glob.glob(os.path.join(args.files_path, "*.png")))

    for fname_in in file_list:
        watermarked = Image.open(fname_in)
        img_size = np.array(watermarked.size)
        watermarked = watermarked.resize(np.array(img_size, dtype=int), Image.Resampling.LANCZOS)

        img_attacked = rinse_nxDiff(watermarked, strength=args.strength, n=args.n_rinse)
        # out_fname = get_output_filename(output_path=output_path, strength=strength, n=n_rinse)
        out_fname = os.path.join(args.output_path, os.path.basename(fname_in))
        print("Saving to " + out_fname)
        if args.orig_img_size is not None:
            img_size = (args.orig_img_size, args.orig_img_size)
        img_attacked = img_attacked.resize(img_size, Image.Resampling.LANCZOS)
