...
if __name__ == '__main__':
    ...
    file_list = sorted(glob.glob(os.path.join(args.watermark_path, "*.png")))
    os.makedirs(args.output_path, exist_ok=True)

    for fname_in in file_list:
        watermarked_img = Image.open(fname_in)
        attacked_img = apply_single_distortion(watermarked_img, args.distortion_type)
        out_fname = os.path.join(args.output_path, os.path.basename(fname_in))
        attacked_img.save(out_fname)